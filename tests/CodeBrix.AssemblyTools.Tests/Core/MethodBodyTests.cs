...
", method);
				...
				(type).Should().Be(method.Body.ThisParameter.ParameterType);
				(method.Body.Instructions [0].Operand).Should().Be(method.Body.ThisParameter);
			});
		}
		...
		public void FilterMaxStack ()
		{
			TestIL ("hello.il", module => {
				...
				Assert.NotNull (method);
				(method.Body.MaxStackSize).Should().Be(2);
			});
		}
		...
		public void BranchOutsideMethod ()
		{
			...
			TestIL ("branch-out.il", module => {
				...
				(leave.OpCode).Should().Be(OpCodes.Leave);
				Assert.Null (leave.Operand);
				(leave.ToString ()).Should().Be("IL_0000: leave");
			}, verify: false);
		}
		...
		public void InsertInstruction ()
		{
			...
			Assert.Null (first.Previous);
			(first.Next).Should().Be(third);
			(third.Previous).Should().Be(first);
