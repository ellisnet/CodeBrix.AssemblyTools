...
public abstract class BaseTestFixture {
	...
	// Upstream Mono.Cecil tests that consume raw-IL fixtures (TestIL / `.il`
	// under Test/Resources/il/) shell out to `ilasm` at test time to compile
	// the `.il` source into a `.dll` that Cecil can then read. CodeBrix.AssemblyTools
	// itself does NOT depend on ilasm at runtime; only these specific tests do.
	// Probe for ilasm once on first use and skip the dependent tests with a
	// clear message when it isn't on PATH. The probe runs `ilasm` with no
	// arguments and treats any successful-launch (even non-zero exit code)
	// as "ilasm exists here".
	static readonly Lazy<bool> _ilasmAvailable = new Lazy<bool>(ProbeForIlasm);

	static bool ProbeForIlasm ()
	{
		try {
	...
	public static void AssertCode (string expected, MethodDefinition method)
	{
		...
		(Normalize (Formatter.FormatMethodBody (method))).Should().Be(Normalize (expected));
	}
