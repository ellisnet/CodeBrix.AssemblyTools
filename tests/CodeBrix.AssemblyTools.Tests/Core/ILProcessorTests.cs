...
public class ILProcessorTests : BaseTestFixture {
	...
	public void InsertBeforeIssue697bis ()
	{
		...
		using (var module = GetResourceModule ("Issue697.dll", parameters)) {
			...
			Assert.False(pathGetterDef.DebugInformation.Scope.Start.IsEndOfMethod);
			foreach (var subscope in pathGetterDef.DebugInformation.Scope.Scopes)
				Assert.False(subscope.Start.IsEndOfMethod);
	...
	[Theory]
	[InlineData(RoundtripType.None, false, false, false)]
	[InlineData(RoundtripType.Pdb, false, false, false)]
	[InlineData(RoundtripType.Pdb, true, false, false)]
	[InlineData(RoundtripType.Pdb, true, false, true)]
	[InlineData(RoundtripType.Pdb, true, true, false)]
	[InlineData(RoundtripType.PortablePdb, false, false, false)]
	[InlineData(RoundtripType.PortablePdb, true, false, false)]
	[InlineData(RoundtripType.PortablePdb, true, false, true)]
	[InlineData(RoundtripType.PortablePdb, true, true, false)]
	...
	public void EditWithDebugInfoButNoLocalScopes()
	{
		...
		(methodBody.Instructions.Count).Should().Be(1);
		(methodBody.Instructions [0].OpCode.Code).Should().Be(Code.Nop);
	...
	static void AssertOpCodeSequence (OpCode [] expected, MethodBody body)
	{
		...
		(opcodes.Length).Should().Be(expected.Length);

		for (int i = 0; i < opcodes.Length; i++)
			(opcodes [i]).Should().Be(expected [i]);
	}
	...
	static void AssertInstructionOffset (Instruction instruction, InstructionOffset instructionOffset)
	{
		if (instructionOffset.IsResolved)
			(instructionOffset.ResolvedInstruction).Should().Be(instruction);
		else
			(instructionOffset.Offset).Should().Be(instruction.Offset);
	}
	...
	static void AssertEndOfScopeOffset (MethodBody methodBody, InstructionOffset instructionOffset, int? index)
	{
		if (index.HasValue)
			...
			Assert.True (instructionOffset.IsEndOfMethod);
	}
	...
	static void AssertStateMachineScope (MethodBody methodBody, int startIndex, int? endIndex)
	{
		...
		Assert.NotNull (stateMachineScope);
		(stateMachineScope.Scopes.Count).Should().Be(1);
	...
	static void AssertAsyncMethodSteppingInfo (MethodBody methodBody, int infoNumber, int yieldIndex, int resumeIndex)
	{
		...
		Assert.NotNull (asyncMethodInfo);
		Assert.True(asyncMethodInfo.Yields.Count > infoNumber);
		(asyncMethodInfo.Resumes.Count).Should().Be(asyncMethodInfo.Yields.Count);
