...
public class ImportReflectionTests : BaseTestFixture {
	...
	public void ImportStringArray ()
	{
		...
		(identity (array)).Should().Be(array);
	}
	...
	public void ImportStringConcat ()
	{
		...
		(concat ("Foo", "Bar")).Should().Be("FooBar");
	}
	...
	public void GeneratedAssemblyCulture ()
	{
		...
		(id.Method.DeclaringType.Assembly.GetName ().CultureInfo.Name).Should().Be("");
	}
	...
	public void ImportGenericField ()
	{
		...
		(get_field (generic)).Should().Be("foo");
	}
	...
	public void ImportGenericMethod ()
	{
		...
		(generic_identity (new Generic<int> (), 42)).Should().Be(42);
	}
	...
	public void ImportGenericMethodSpec ()
	{
		...
		(gen_spec_id (new Generic<string> (), 42)).Should().Be(42);
	}
	...
	public void ImportGenericTypeDefOrOpen ()
	{
		using (var module = typeof (Foo<>).ToDefinition ().Module) {
			...
			// was previously: (foo_def.FullName).Should().Be("Mono.Cecil.Tests.ImportReflectionTests/Foo`1");
			(foo_def.FullName).Should().Be("CodeBrix.AssemblyTools.Tests.Core.ImportReflectionTests/Foo`1");
			// was previously: (foo_open.FullName).Should().Be("Mono.Cecil.Tests.ImportReflectionTests/Foo`1<TFoo>");
			(foo_open.FullName).Should().Be("CodeBrix.AssemblyTools.Tests.Core.ImportReflectionTests/Foo`1<TFoo>");
		}
	}
	...
	public void ImportGenericTypeFromContext ()
	{
		...
		using (var module = foo_def.Module) {
			...
			// was previously: (generic_foo.FullName).Should().Be("Mono.Cecil.Tests.ImportReflectionTests/Generic`1<System.Collections.Generic.List`1<TFoo>>");
			(generic_foo.FullName).Should().Be("CodeBrix.AssemblyTools.Tests.Core.ImportReflectionTests/Generic`1<System.Collections.Generic.List`1<TFoo>>");
		}
	}
	...
	public void ImportGenericTypeDefFromContext ()
	{
		...
		using (var module = foo_def.Module) {
			...
			// was previously: (generic_foo.FullName).Should().Be("Mono.Cecil.Tests.ImportReflectionTests/Generic`1<Mono.Cecil.Tests.ImportReflectionTests/Foo`1<TFoo>>");
			(generic_foo.FullName).Should().Be("CodeBrix.AssemblyTools.Tests.Core.ImportReflectionTests/Generic`1<CodeBrix.AssemblyTools.Tests.Core.ImportReflectionTests/Foo`1<TFoo>>");
		}
	}
	...
	public void ImportArrayTypeDefFromContext ()
	{
		...
		using (var module = foo_def.Module) {
			...
			// was previously: (array_foo.FullName).Should().Be("Mono.Cecil.Tests.ImportReflectionTests/Foo`1<TFoo>[]");
			(array_foo.FullName).Should().Be("CodeBrix.AssemblyTools.Tests.Core.ImportReflectionTests/Foo`1<TFoo>[]");
		}
	}
	...
	public void ImportGenericFieldFromContext ()
	{
		...
		using (var module = foo_def.Module) {
			...
			// was previously: (generic_field.FullName).Should().Be("T Mono.Cecil.Tests.ImportReflectionTests/Generic`1<System.Collections.Generic.List`1<TFoo>>::Field");
			(generic_field.FullName).Should().Be("T CodeBrix.AssemblyTools.Tests.Core.ImportReflectionTests/Generic`1<System.Collections.Generic.List`1<TFoo>>::Field");
		}
	}
	...
	public void ImportGenericMethodFromContext ()
	{
		...
		using (var module = foo_def.Module) {
			...
			// was previously: (generic_method.FullName).Should().Be("T Mono.Cecil.Tests.ImportReflectionTests/Generic`1<System.Collections.Generic.List`1<TFoo>>::Method(T)");
			(generic_method.FullName).Should().Be("T CodeBrix.AssemblyTools.Tests.Core.ImportReflectionTests/Generic`1<System.Collections.Generic.List`1<TFoo>>::Method(T)");
		}
	}
	...
	public void ImportMethodOnOpenGenericType ()
	{
		using (var module = typeof (Generic<>).ToDefinition ().Module) {
			...
			// was previously: (method.FullName).Should().Be("T Mono.Cecil.Tests.ImportReflectionTests/Generic`1<T>::Method(T)");
			(method.FullName).Should().Be("T CodeBrix.AssemblyTools.Tests.Core.ImportReflectionTests/Generic`1<T>::Method(T)");
		}
	}
	...
	public void ImportGenericMethodOnOpenGenericType ()
	{
		using (var module = typeof (Generic<>).ToDefinition ().Module) {
			...
			// was previously: (generic_method.FullName).Should().Be("TS Mono.Cecil.Tests.ImportReflectionTests/Generic`1<T>::GenericMethod(T,TS)");
			(generic_method.FullName).Should().Be("TS CodeBrix.AssemblyTools.Tests.Core.ImportReflectionTests/Generic`1<T>::GenericMethod(T,TS)");

			generic_method = module.ImportReference (typeof (Generic<>).GetMethod ("GenericMethod"), generic_method);

			// was previously: (generic_method.FullName).Should().Be("TS Mono.Cecil.Tests.ImportReflectionTests/Generic`1<T>::GenericMethod<TS>(T,TS)");
			(generic_method.FullName).Should().Be("TS CodeBrix.AssemblyTools.Tests.Core.ImportReflectionTests/Generic`1<T>::GenericMethod<TS>(T,TS)");
		}
	}
