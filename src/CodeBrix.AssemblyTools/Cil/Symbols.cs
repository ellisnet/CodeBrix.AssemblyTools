...
namespace CodeBrix.AssemblyTools.Cil; //was previously: Mono.Cecil.Cil;
...
static class SymbolProvider {
	...
	{
		// CodeBrix.AssemblyTools port: upstream Mono.Cecil split Native-PDB and
		// MDB support across sibling assemblies (Mono.Cecil.Pdb, Mono.Cecil.Mdb)
		// and resolved their provider types by name: "Mono.Cecil" + "." +
		// namespace + "." + kind + suffix. Our port merges those assemblies into
		// this one under CodeBrix.AssemblyTools.{Pdb,Mdb}.*, so the lookup
		// prefix must match this assembly's root namespace.
		return "CodeBrix.AssemblyTools" + "." + GetSymbolNamespace (kind) + "." + kind + name;
	}
