...
using CodeBrix.AssemblyTools.Security.Cryptography;
...
static class CryptoService {
	...
	// Upstream 0.11.6 (commit dff01d9) switched from `SHA1Managed` to
	// `SHA1CryptoServiceProvider` so that FIPS-policy hosts would accept the
	// hash. On .NET 6+ `SHA1CryptoServiceProvider` is obsoleted (SYSLIB0021)
	// and the recommended replacement is `SHA1.Create()`, which returns an
	// implementation that is FIPS-compliant at runtime.
